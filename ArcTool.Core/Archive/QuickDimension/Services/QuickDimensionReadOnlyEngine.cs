...
{
    /// <summary>
    /// Production read-only Quick Dimension engine for Phase 2.7.
    ...
    public static class QuickDimensionReadOnlyEngine
    {
            ...
            QuickDimensionOptions? options = null)
        {
            ...
            if (!IsSupportedPlanView(view))
            {
                ...
                return BuildFinalResult(lineContext, rawCandidates, diagnostics, collectedCounts, effectiveOptions, wallAxisAggregationTrace, timingTrace);
            }

            // Wall-axis projection model (ADR-2026-06-11 + Session 2.7): the selected wall IS the axis.
            // Gather references from its picked-side anchors plus hosted Door/Window jambs, all projected onto
            // that axis. Grid and other non-host sources are intentionally excluded from this main flow.
            if (lineContext.IsWallAxis)
            {
                wallAxisAggregationTrace = new QuickDimensionWallAxisAggregationTrace();
                timingTrace = new QuickDimensionCollectionTimingTrace();
                CollectWallAxisCandidates(doc, view, lineContext, effectiveOptions, rawCandidates, diagnostics, collectedCounts, wallAxisAggregationTrace, timingTrace);
                return BuildFinalResult(lineContext, rawCandidates, diagnostics, collectedCounts, effectiveOptions, wallAxisAggregationTrace, timingTrace);
            }
            ...
            return BuildFinalResult(lineContext, rawCandidates, diagnostics, collectedCounts, effectiveOptions, wallAxisAggregationTrace);
        }

        /// <summary>
        /// Wall-axis projection model orchestration: resolves the selected wall from the context, then merges
        /// its picked-side anchors (via the directional side-face boundary resolver) and its hosted Door/Window
        /// jambs, all projected onto the wall axis.
        /// </summary>
        private static void CollectWallAxisCandidates(
            ...
            QuickDimensionWallAxisAggregationTrace wallAxisAggregationTrace,
            QuickDimensionCollectionTimingTrace timingTrace)
        {
            ...
            try
            {
                ...
                if (effectiveOptions.IncludeWalls)
                {
                    Stopwatch wallAnchorStopwatch = Stopwatch.StartNew();
                    QuickDimensionReadOnlyResult endAnchorResult = QuickDimensionWallCandidateCollector.CollectSelectedWallEndAnchors(doc, lineContext, selectedWall, effectiveOptions);
                    wallAnchorStopwatch.Stop();
                    timingTrace.WallEndAnchorCollectionMilliseconds = wallAnchorStopwatch.Elapsed.TotalMilliseconds;
                    ...
                    // Wall-only mid-run aggregator: collect accepted joining-wall reference hits along the selected wall axis.
                    // Uses the proven spike resolver's anchor result to filter mid-run stations (ADR-2026-07-19A gates).
                    XYZ? nullableSideNormal = lineContext.SideNormal;
                    if (endAnchorResult.Candidates.Count == 2 && lineContext.SideSign != 0 && nullableSideNormal != null)
                    {
                        Stopwatch midRunStopwatch = Stopwatch.StartNew();
                        XYZ sidePickPoint = lineContext.FirstPoint + nullableSideNormal;
                        QuickDimensionWallSpikeResult spikeResult = QuickDimensionWallReferenceProbeService.RunWallReferenceProbe(selectedWall, sidePickPoint);
                        if (spikeResult.Succeeded)
                        {
                            List<QuickDimensionCandidate> midRunCandidates = QuickDimensionWallAxisAggregatorService.CollectMidRunCandidates(
                                doc,
                                view,
                                selectedWall,
                                spikeResult,
                                lineContext,
                                diagnostics,
                                wallAxisAggregationTrace);
                            rawCandidates.AddRange(midRunCandidates);

                            if (collectedCounts.ContainsKey(QuickDimensionSourceType.Wall))
                            {
                                collectedCounts[QuickDimensionSourceType.Wall] += midRunCandidates
                        ...
                        midRunStopwatch.Stop();
                        timingTrace.MidRunAggregationMilliseconds = midRunStopwatch.Elapsed.TotalMilliseconds;
                    }
                }
                else
                {
                ...
                if (effectiveOptions.IncludeDoors || effectiveOptions.IncludeWindows)
                {
                    Stopwatch openingStopwatch = Stopwatch.StartNew();
                    QuickDimensionReadOnlyResult openingResult = QuickDimensionDoorWindowCandidateCollector.CollectOpeningsAlongWallAxis(doc, view, lineContext, selectedWall.Id, effectiveOptions);
                    openingStopwatch.Stop();
                    timingTrace.OpeningCollectionMilliseconds = openingStopwatch.Elapsed.TotalMilliseconds;
            ...
            {
                totalStopwatch.Stop();
                timingTrace.TotalWallAxisCollectionMilliseconds = totalStopwatch.Elapsed.TotalMilliseconds;
            }
        }
        ...
        private static IReadOnlyList<QuickDimensionCandidate> RemoveDuplicateStations(
            ...
            QuickDimensionCollectionTimingTrace? timingTrace = null)
        {
            Stopwatch duplicateStationStopwatch = Stopwatch.StartNew();
            ...
            {
                duplicateStationStopwatch.Stop();
                if (timingTrace != null)
                {
                    timingTrace.DuplicateStationReductionMilliseconds = duplicateStationStopwatch.Elapsed.TotalMilliseconds;
                }
            }
        }
