...
{
    /// <summary>
    /// Session 2.6 read-only smoke command for the wall-axis projection model: selects one host Wall,
    /// picks the placement side, and reports ordered wall-end/opening candidates without creating dimensions.
    /// </summary>
    ...
    public class QuickDimensionReadOnlySummaryCommand : IExternalCommand
    {
        ...
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            if (!IsSupportedPlanView(activeView))
            {
                RevitTaskDialog.Show(
                    ...
                    "Quick Dimension Session 2.6 supports active Plan Views only.\n\n" +
            ...
            try
            {
                ...
                RevitTaskDialog.Show(
                    "ArcTool - Quick Dimension Read-Only Summary",
                    BuildSummaryMessage(result, activeView, selectedWall));
        ...
        private static string BuildSummaryMessage(QuickDimensionReadOnlyResult result, RevitView activeView, Wall selectedWall)
        {
