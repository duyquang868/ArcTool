...
namespace ArcTool.Core.Services
{
    ...
    public static class QuickDimensionWallCandidateCollector
    {
        ...
        /// <summary>
        /// Wall-axis projection model (ADR-2026-06-11): collects the selected wall's two END faces as chain anchors,
        /// projecting their reference points onto the wall axis. The wall body itself is the axis, so there is no
        /// side-face intersection here; the end faces give the start/end of the dimension chain along the wall.
        /// </summary>
            ...
            QuickDimensionOptions? options = null)
        {
            ...
            TryCollectWallEndAnchors(selectedWall, lineContext, effectiveOptions, rawCandidates, diagnostics);
            ...
            List<QuickDimensionDiagnostic> diagnostics)
        {
            ...
            try
            {
                ...
                if (wall.Location is not LocationCurve locationCurve || locationCurve.Curve is not Line)
                {
                    diagnostics.Add(new QuickDimensionDiagnostic(
                        QuickDimensionDiagnosticSeverity.Warning,
                        QuickDimensionRejectedReason.ArcWallUnsupported,
                        $"Skipped Wall '{displayName}' because only straight (Line) host walls are supported as the dimension axis.",
                        wall.Id, QuickDimensionSourceType.Wall, displayName));
                    return;
                }

                IReadOnlyList<WallEndFaceCandidate> endFaceCandidates = GetBestWallEndFaceCandidates(
                    wall,
                    lineContext,
                    options.ProjectionTolerance,
                    options.WallEndStationTolerance);

                if (endFaceCandidates.Count < 2)
                {
                    double stationToleranceMm = UnitUtils.ConvertFromInternalUnits(options.WallEndStationTolerance, UnitTypeId.Millimeters);
                    diagnostics.Add(new QuickDimensionDiagnostic(
                        QuickDimensionDiagnosticSeverity.Warning,
                        QuickDimensionRejectedReason.MissingReference,
                        $"Skipped Wall '{displayName}' because two distinct physical wall end-cap references could not be resolved (station tolerance = {stationToleranceMm:0.##} mm). Opening jamb faces are expected to sit between the physical end caps and are not used as wall anchors.",
                        wall.Id, QuickDimensionSourceType.Wall, displayName));
                    return;
                }

                string typeName = wall.WallType?.Name ?? string.Empty;
                foreach (WallEndFaceCandidate endFaceCandidate in endFaceCandidates)
                {
                    candidates.Add(new QuickDimensionCandidate(
                        wall.Id, QuickDimensionSourceType.Wall,
                        $"{displayName} [{endFaceCandidate.Label}]",
                        endFaceCandidate.Reference, QuickDimensionReferenceStrategy.WallEndFace,
                        endFaceCandidate.ProjectedPoint, endFaceCandidate.ParameterOnDimensionLine,
                        typeName: typeName));
                }
            }
            catch (Exception ex)
        ...
        private static IReadOnlyList<WallEndFaceCandidate> GetBestWallEndFaceCandidates(
            Wall wall,
            QuickDimensionLineContext lineContext,
            double projectionTolerance,
            double wallEndStationTolerance)
        {
            var faceCandidates = new List<WallEndFaceCandidate>();
            ...
            {
                return Array.Empty<WallEndFaceCandidate>();
            }
            ...
            foreach (GeometryObject geometryObject in geometryElement)
            {
                ...
                {
                    CollectWallEndFaceCandidates(solid, lineContext, projectionTolerance, faceCandidates);
                }
                else if (geometryObject is GeometryInstance geometryInstance)
                {
                    ...
                    foreach (GeometryObject instanceObject in instanceGeometry)
                    {
                        ...
                        {
                            CollectWallEndFaceCandidates(instanceSolid, lineContext, projectionTolerance, faceCandidates);
                        }
                    }
                }
            }

            if (faceCandidates.Count < 2)
            {
                return Array.Empty<WallEndFaceCandidate>();
            }

            // Wall LocationCurve endpoints can lie on centerlines at joins. The physical dimension anchor
            // is the solid end cap (the wall-corner face), not necessarily station 0 or Length of the
            // LocationCurve. Among wall-direction-aligned cap/reveal faces, opening jamb faces sit between
            // the two physical wall ends, so min/max projected stations identify the true solid caps.
            WallEndFaceCandidate startCandidate = faceCandidates
                .OrderBy(candidate => candidate.ParameterOnDimensionLine)
                .First();

            WallEndFaceCandidate endCandidate = faceCandidates
                .OrderByDescending(candidate => candidate.ParameterOnDimensionLine)
                .First();

            if (ReferenceEquals(startCandidate, endCandidate) ||
                Math.Abs(endCandidate.ParameterOnDimensionLine - startCandidate.ParameterOnDimensionLine) <= wallEndStationTolerance)
            {
                return Array.Empty<WallEndFaceCandidate>();
            }
            ...
            return new[]
            {
                startCandidate.WithLabel("Start End"),
                endCandidate.WithLabel("Finish End")
            };
        }

        private static void CollectWallEndFaceCandidates(
            Solid solid,
            QuickDimensionLineContext lineContext,
            double tolerance,
            List<WallEndFaceCandidate> result)
        {
            ...
            foreach (Face face in solid.Faces)
            {
                ...
                if (!IsSupportedWallEndFaceNormal(faceNormal, lineContext.Direction, tolerance))
                {
                    continue;
                }
                ...
                // Compute parameter directly without the picked-span guard so wall joins that push the
                // end cap slightly beyond [0, Length] are not silently dropped. The outer filter uses
                // WallEndStationTolerance to keep only faces near the true wall endpoints.
                double parameterOnDimensionLine = lineContext.ProjectParameter(faceCentroid);
                XYZ projectedPoint = lineContext.Evaluate(parameterOnDimensionLine);

                result.Add(new WallEndFaceCandidate(
                    string.Empty,
                    reference,
                    projectedPoint,
        ...
        private static bool IsSupportedWallEndFaceNormal(XYZ faceNormal, XYZ wallDirection, double tolerance)
        {
            if (!QuickDimensionGeometryService.IsFinite(faceNormal) || !QuickDimensionGeometryService.IsFinite(wallDirection))
            {
                return false;
            }
            ...
            double alignment = Math.Abs(planarNormal.DotProduct(wallDirection));
            return alignment >= 1.0 - Math.Max(tolerance, 1e-3);
        }
        ...
        private sealed class WallEndFaceCandidate
        {
            public WallEndFaceCandidate(
                string label,
                Reference reference,
                XYZ projectedPoint,
                double parameterOnDimensionLine)
            {
                ...
                Reference = reference ?? throw new ArgumentNullException(nameof(reference));
                ProjectedPoint = projectedPoint ?? throw new ArgumentNullException(nameof(projectedPoint));
                ParameterOnDimensionLine = parameterOnDimensionLine;
            }
            ...
            public WallEndFaceCandidate WithLabel(string label)
            {
                return new WallEndFaceCandidate(label, Reference, ProjectedPoint, ParameterOnDimensionLine);
            }
        }
    }
}
