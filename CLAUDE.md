...
> Last updated: 2026-05-14 — Session 7.2: Excel to Revit V3.0 + Export Pipeline Runtime Fixes ✅ COMPLETE — ExcelInteropService V6.1 + ExcelSyncEngine error surfacing
...
│   ├── Services/
│   │   ├── ExcelInteropService.cs      ← V6.1 ✅ STABLE — PDF pipeline + native runtime probe (pdfium/libSkiaSharp)
│   │   ├── ArcToolSettingsService.cs   ← V1.0 ✅ STABLE — Load/Save JSON, atomic write
│   │   └── ExcelSyncEngine.cs         ← V1.1 ✅ STABLE — CheckForChanges/ExecuteUpdate/GetOrCreateView + user-facing export errors
│   ├── UI/
│   │   ├── FilterWindow.xaml
...
### E. Excel Export Engine — `ExcelInteropService.cs` (V6.1 ✅ STABLE — Session 7.2)
- Pipeline cũ (`CopyPicture`/Clipboard) bị thay thế hoàn toàn
- Pipeline mới: Excel → PDF (`ExportAsFixedFormat`) → PDFtoImage → PNG → SkiaSharp crop
- Lý do đổi pipeline: `CopyPicture(xlScreen)` bị giới hạn ~20 dòng do virtual DC trong hidden mode
- Thư viện mới: `PDFtoImage` 5.2.1 (MIT)
- `SkiaSharp` đã có sẵn trong project — tái sử dụng để crop white margins
- `FIXED_SCALE_FACTOR 35x` bị xóa — scale giờ do PDFium render ở 300 DPI quyết định
- Có `IDisposable`, COM release đúng thứ tự child → parent
- **Native runtime probe đã implement để tránh Revit crash khi deploy thiếu file:**
  - `EnsurePdfiumLoaded()` probe theo thứ tự: `pdfium.dll` cạnh assembly → `native\pdfium.dll` → `runtimes\win-x64|x86|arm64\native\pdfium.dll`
  - `EnsureSkiaSharpLoaded()` probe theo thứ tự: `libSkiaSharp.dll` cạnh assembly → `native\libSkiaSharp.dll` → `runtimes\win-x64|x86|arm64\native\libSkiaSharp.dll`
  - Nếu thiếu `pdfium.dll` → export fail mềm (`return false`), caller hiện lỗi thay vì crash AppDomain
  - Nếu thiếu `libSkiaSharp.dll` → bỏ qua bước crop trắng, giữ PNG gốc đã render để lệnh vẫn tiếp tục
- **Public API hiện có / dự kiến giữ lại:**
  ...
  - `ExportRegion(sheetName, regionName, outputPath)` ✅ V5.3/V6.1 — export theo sheet + region (Named Range → Print Area → UsedRange fallback) + guard native runtime
...
### J. Excel Sync Engine — `Services/ExcelSyncEngine.cs` (V1.1 ✅ STABLE — Session 7.2)
...
- **Public API:**
  ...
  - `ExecuteUpdate(ExcelMapping, Document, List<ExcelMapping>)` → `bool`
    ...
    - Các lỗi export/user-facing quan trọng (`OpenFile` fail, `ExportRegion` fail, PNG không tạo được) → `throw InvalidOperationException` để UI hiện `TaskDialog`, không còn silent no-op
    - Revit API lỗi, `IOException`, lỗi không xác định → propagate lên caller
...
- **Quyết định thiết kế V1.1 đã chốt:**
  ...
  - `StoredWidth/StoredHeight` trong JSON được lưu theo **millimeters**, không còn giả định internal feet ở layer model/service boundary
...
| BUG-E9 | ExcelInteropService | Cache `_activeSheet` như COM field lâu dài gây nguy cơ trỏ vào RCW/COM object không còn hợp lệ sau các luồng duyệt worksheet. Fix: bỏ cache `_activeSheet`, lấy `ActiveSheet` theo local scope và release ngay trong `finally` |
| BUG-E10 | ExcelInteropService | Revit crash khi `PDFtoImage` không resolve được native `pdfium.dll`. Fix: thêm `EnsurePdfiumLoaded()` probe theo assembly root / `native` / `runtimes\win-*\native`, fail mềm thay vì để AppDomain crash |
...
| `xlPaperEsheet` cho PageSetup | 864×1118mm — khổ lớn nhất trong `XlPaperSize` enum | Cần crop margins trắng sau khi render PDF |
| `FitToPagesWide=1 + FitToPagesTall=1 + Zoom=false + margin=0` | Ép toàn bộ vùng export vào 1 trang PDF | Tỉ lệ cuối cùng phụ thuộc PDF render DPI thay vì scale tay |
| Không Save & Restore PageSetup | File Excel là dữ liệu thô riêng biệt, không phải file pháp lý | Layout in của workbook sẽ bị tool sửa trong lúc export |
| `FIXED_SCALE_FACTOR 35x` bị xóa | Scale cũ chỉ phục vụ bitmap pipeline; giờ scale do PDFium DPI=300 | Không còn constant nội bộ để tinh chỉnh scale tay |
...
| Native runtime probe cho PDF/Skia | Revit add-in không phải lúc nào cũng resolve native DLL theo `.deps.json` như app thường; cần probe thủ công | Thêm code load runtime, đổi lại deploy phải mang theo `pdfium.dll` và `libSkiaSharp.dll` đúng kiến trúc |
| Skip crop nếu thiếu `libSkiaSharp.dll` | Tạo PNG gốc còn tốt hơn crash toàn bộ Revit ở bước hậu xử lý | Có thể còn viền trắng nếu deploy thiếu native SkiaSharp |
| Export/user-facing errors trong ExecuteUpdate() phải throw | `return false` làm UI im lặng, user tưởng lệnh không chạy | Caller phải bắt `InvalidOperationException` và hiện dialog rõ ràng |
...
### 6.8 ExcelInteropService — Public API Đầy Đủ (V6.1 ✅)
...
// [V5.3/V6.1] Export theo sheet + region cụ thể
// regionName = null/empty → fallback Print Area → UsedRange tự động
// Native runtime guard: probe pdfium/libSkiaSharp trước khi đi vào nhánh native
public bool ExportRegion(string sheetName, string regionName, string outputPath)

// Dispose: workbook → app, GC.Collect()
public void Dispose()
```

**Pattern sử dụng V6.1 trong ExcelSyncEngine:**
```csharp
...
Phase 1 — Models & Services (không phụ thuộc UI)
  ...
  [x] Nâng cấp Services/ExcelInteropService.cs → V6.1 ✅ Session 7.2 — runtime crash fixes + deploy verification
        NOTE: bỏ cache `_activeSheet`, dùng local `ActiveSheet` scope + release ngay
        NOTE: thêm `EnsurePdfiumLoaded()` probe `pdfium.dll` ở assembly root / `native` / `runtimes\win-*\native`
        NOTE: thêm `EnsureSkiaSharpLoaded()` probe `libSkiaSharp.dll`; thiếu lib → skip crop, giữ PNG gốc
        NOTE: deploy thực tế đã verify thành công khi folder add-in chứa đủ `pdfium.dll` + `libSkiaSharp.dll`
...
- Phase 1C:
  - ExcelInteropService V5.3 ✅ STABLE
  - ExcelInteropService V6.0 ⏳ PLANNED — PDF pipeline thay thế CopyPicture
...
// 12. Native runtime deploy cho Excel PDF pipeline
// Folder add-in chạy thực tế PHẢI có đủ pdfium.dll và libSkiaSharp.dll đúng kiến trúc.
// Ưu tiên probe theo thứ tự:
//   1) cạnh ArcTool.Core.dll
//   2) native\pdfium.dll / native\libSkiaSharp.dll
//   3) runtimes\win-x64|x86|arm64\native\...
// Thiếu pdfium.dll → export fail mềm + hiện dialog; thiếu libSkiaSharp.dll → skip crop, giữ PNG gốc
```
...
| `Worksheet.ExportAsFixedFormat(xlTypePDF, path)` | Export sheet → PDF không dùng clipboard |
| `XlPaperSize.xlPaperEsheet` | 864×1118mm — khổ lớn nhất trong enum, dùng cho PDF export |
| `PDFtoImage.Conversion.SavePng()` | Render PDF page → PNG 300 DPI (PDFium engine) |
